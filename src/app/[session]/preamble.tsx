...
function chooseRenderFn(stage: any): renderT {
  ...
  if (stage.field === "bonus") {
    return (bonus: BonusCard): React.ReactNode => BonusTicket({ bonus: bonus });
  }
  return (hs: HyperSet): React.ReactNode => ShowWordBooster({ hs: hs });
}
  ...
  if (preamble.stagekey === "opponent") {
    ...
    return opts.map((o) =>
      OptionTile({
        ...
        handler: () => statefn(async (p: Preamble) => ChooseOpponent(p, o)),
      }),
    );
  }
  ...
  if (preamble.stagekey === "ability") {
    ...
    return opts.map((o) =>
      OptionTile({
        ...
        handler: () => statefn(async (p: Preamble) => ChooseAbility(p, o)),
      }),
    );
  }
  ...
  if (preamble.stagekey === "bonus") {
    ...
    return opts.map((o) =>
      OptionTile({
        ...
        handler: () => statefn(async (p: Preamble) => ChooseBonus(p, o)),
      }),
    );
  }
  ...
  if (preamble.stagekey === "word") {
    ...
    return opts.map((o) =>
      OptionTile({
        ...
        handler: () => statefn(async (p: Preamble) => ChooseWord(p, o)),
      }),
    );
  }
...
export function ShowPreamble({
  ...
  const title =
    myPreamble.stagekey === "opponent"
      ? "Select Your Opponent"
      : myPreamble.stagekey === "ability"
        ? "Select An Ability"
        : myPreamble.stagekey === "bonus"
          ? "Select A Bonus"
          : myPreamble.stagekey === "word"
            ? "Select A Word Booster"
            : "Un-Titled";
    ...
    <main className="flex-1 flex flex-col gap-2 mx-2 items-center">
      ...
      <div className="grid">
        <AnimatePresence>
          ...
          >
            <div className="text-amber-300 text-4xl font-light">{title}</div>
...
function OpponentMugshot({ opponent }: { opponent: Opponent }) {
  return (
...
function AbilityTicket({ ability }: { ability: AbilityCard }) {
...
function BonusTicket({ bonus }: { bonus: BonusCard }) {
  return (
...
function ShowWordBooster({ hs }: { hs: HyperSet }) {
  const stride = 3;
