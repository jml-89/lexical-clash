...
export type PreambleStageType = Opponent | AbilityCard | BonusCard | HyperSet;
...
async function assembleWordBoosters(
  g: PreambleSetup,
): Promise<Map<string, HyperSet>> {
  const xs = await g.kb.candidates(g.level * 200, (g.level + 1) * 300, 10, 5);
...
export async function ChooseAbility(
  p: Preamble,
  a: AbilityCard,
): Promise<void> {
  p.ability.choice = a;
